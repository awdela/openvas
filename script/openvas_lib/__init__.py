...
def report_parser(path_or_file, ignore_log_info=True):
    print("input param is %s"%path_or_file)
    """
    This functions transform XML OpenVas file report to OpenVASResult object structure.
    ...
    pdb.set_trace()
    try:
        print("path or file is ******%s"%path_or_file)
    ...
    for l_results in xml.findall(".//result"):
        ...
        for l_val in l_results.getchildren():
            ...
            # --------------------------------------------------------------------------
            # Common properties: subnet, host, threat, description
            # --------------------------------------------------------------------------
            if l_tag in ("subnet", "host", "name","comment","threat","severity","creation_time",\
                        "modification_time","scan_nvt_version","description"):
            ...
            # --------------------------------------------------------------------------
                ...
                # Looking for qod 
                print("l_val is =========%s"%l_val)
                print("l_val.tag is =========%s"%l_val.tag)
                
                qod_obj = l_val.getchildren()
                if qod_obj:
                    for qod_val in qod_obj:
                        print("val in qod is =========%s"%qod_val)
                        print("val in name is =========%s"%qod_val.tag)
                        print("val in text is =========%s"%qod_val.text)
                        if qod_val.tag=='value':
                            try:
                                setattr(l_partial_result, l_tag, qod_val.text)
                            except (TypeError, ValueError) as e:
                                logging.warning(
                                    "%s is not a valid value for %s property in %s vulnerability. skipping vulnerability..."
                                    % (l_val.text,
            ...
            # --------------------------------------------------------------------------
                ...
                if l_port:
                    ...
                    l_number = int(l_port.group(1))
                    l_proto = l_port.group(2)

                    try:
                        l_partial_result.port = OpenVASPort(l_number,
                                                            l_proto)
                    except (TypeError, ValueError) as e:
                    ...
                    if l_port:
                        ...
                        try:
                            l_partial_result.port = OpenVASPort(0, l_proto)
                        except (TypeError, ValueError) as e:
                            loging.warning("%s is not a valid port for %s vulnerability. skipping vulnerability..."
                                            % (l_val.text,
                                               l_vid))
                            logging.debug(e)
                            continue
            ...
            # --------------------------------------------------------------------------
                ...
                for l_nvt in l_val.getchildren():
                    ...
                    if l_nvt_tag in l_nvt_symbols:
                        ...
                        print("l_nvt_tag is =========%s"%l_nvt_tag)
                        if l_nvt_tag=='cert':
                            cert_content = ""
                            cert_list=[]   
                            if l_nvt.getchildren():
                                print ("l_nvt children is =========%s"%l_nvt.getchildren())
                                for tmp in l_nvt.getchildren():
                                    print ("l_nvt single children is =========%s"%tmp.attrib['id'])
                                    cert_list.append(tmp.attrib['id']) 
                                cert_content = ",".join(cert_list) 
                                print("cert content is =========%s"%cert_content)
                            try:
                                setattr(l_nvt_object, l_nvt_tag, cert_content)
                            except (TypeError, ValueError) as e:
                                logging.warning(
                            ...
                            if l_nvt.text.startswith("NO"):
                                ...
                                #print("l_val is =========%s"%l_nvt.text)
                                
                                #pdb.set_trace()
                                try:
                                    setattr(l_nvt_object, "raw_tags", l_nvt.text)
                                except TypeError as e:
                                    logging.warning(
                                        "Empty value is not a valid NVT value for %s property in %s vulnerability. skipping vulnerability..."
                                        % (l_nvt_tag,
                                    ...
                                    logging.debug(e)
                                    continue
                                #print("impact is =========%s"%l_nvt_object.impact)
                                #print("summary is =========%s"%l_nvt_object.summary)
                                #print("solution is =========%s"%l_nvt_object.solution)
...
class VulnscanManager(object):
    ...
    # ----------------------------------------------------------------------
    def get_results(self, task_id,ignore_log_info=True):
        """
        ...
        try:
            print ("in __init__.py task id is %s"%(task_id))
            #m_response = self.__manager.get_results(task_id)
            report_id = self.__manager.get_report_id(task_id)
            print ("report id is ============%s"%report_id)
            m_response = self.__manager.get_report_xml(report_id)
            print ("in __init__.py m_response is %s"%(m_response))
        except ServerError as e:
            raise VulnscanServerError("Can't get the results for the task %s. Error: %s" % (task_id, e.message))
        return report_parser(m_response,ignore_log_info)
